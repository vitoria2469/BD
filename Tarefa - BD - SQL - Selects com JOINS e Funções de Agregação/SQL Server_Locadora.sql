...
USE locadora
GO
SELECT c.num_cadastro, c.nome, f.titulo, d.data_fabricacao,
loc.valor
FROM cliente AS c
INNER JOIN locacao loc
ON c.num_cadastro = loc.cliente_num
INNER JOIN dvd d
ON d.num = loc.dvd_num
INNER JOIN filme f
ON f.id = d.filme_id
WHERE d.data_fabricacao IN --- Sub Consulta que retorna a maior(mais recente) data de fabricação
(
		SELECT MAX(dvd.data_fabricacao) FROM dvd 

)
GROUP BY c.num_cadastro, c.nome, f.titulo, d.data_fabricacao,
loc.valor
...
SELECT c.num_cadastro, c.nome, CONVERT(char(10),loc.data_locacao,103) AS dt_locacao,
COUNT(loc.cliente_num) as qtd  
FROM cliente c, locacao loc, dvd d, filme f
WHERE c.num_cadastro = loc.cliente_num
AND d.num = loc.dvd_num
AND f.id = d.filme_id
GROUP BY c.num_cadastro, c.nome, loc.data_locacao
...
CONVERT(CHAR(10), loc.data_locacao,103) AS dt_locacao,
...
CONVERT(CHAR(8), loc.data_locacao, 103) as dt_locacao
FROM cliente c, filme f, locacao loc, dvd d
WHERE c.num_cadastro = loc.cliente_num
AND d.num = loc.dvd_num
AND f.id = d.filme_id
AND c.num_cadastro IN ( --- Sub consulta para buscar os clientes que alugaram +2 filmes simultaneamente
						SELECT c.num_cadastro FROM cliente c, locacao loc, filme f, dvd d
						WHERE c.num_cadastro = loc.cliente_num
						AND d.num = loc.dvd_num
						AND f.id = d.filme_id
						GROUP BY c.num_cadastro
						HAVING COUNT(loc.data_locacao) > 2 --- Conta quantas vezes a data foi repetida e verifica se foi + que 2 vezes
)
GROUP BY c.num_cadastro, c.nome, c.logradouro, c.num, loc.data_locacao
